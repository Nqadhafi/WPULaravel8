...
<div class="card mb-3">
    ...
    <div class="card-body text-center">
      ...
      <h6>Ditulis oleh: {{ $blogposts[0]->author }} in <a href="/blog?category={{ $blogposts[0]->category->slug }} " class="text-decoration-none">{{ $blogposts[0]->category->name }} </a><small class="text-body-secondary">{{ $blogposts[0]->created_at->diffForHumans() }}</small></h6>
          ...
          <div class="position-absolute px-3 py-2" style="background-color: rgba(0, 0, 0, 0.7); color: white; font-size: 12px; border-radius: 5px;">
           <a href="/blog?category={{ $post->category->slug }}" class="text-decoration-none text-white">{{ $post->category->name }}</a> 
            ...
            <div class="card-body">
              ...
              <h6>Ditulis oleh: {{ $post->author }} in <a href="/blog?category={{ $post->category->slug }}" class="text-decoration-none">{{ $post->category->name }}</a><small class="text-body-secondary">{{ $post->created_at->diffForHumans() }}</small></h6>
